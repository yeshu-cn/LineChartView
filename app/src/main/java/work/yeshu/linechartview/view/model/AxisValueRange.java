...
/**
 * Partial copy of android.graphics.Rect but here the top should be greater then the bottom. AxisValueRange holds 4 float
 ...
 * bottom is less than top). AxisValueRange implements Parcerable.
 */
public class AxisValueRange implements Parcelable {
    ...
    public float right;
    public float bottom;
    public static final Creator<AxisValueRange> CREATOR = new Creator<AxisValueRange>() {
        /**
         * Return a new viewport from the data in the specified parcel.
         */
        public AxisValueRange createFromParcel(Parcel in) {
            AxisValueRange v = new AxisValueRange();
            v.readFromParcel(in);
            return v;
        }
         ...
         */
        public AxisValueRange[] newArray(int size) {
            return new AxisValueRange[size];
        }
    };

    /**
     * Create a new empty AxisValueRange. All coordinates are initialized to 0.
     */
    public AxisValueRange() {
    }
     ...
     */
    public AxisValueRange(float left, float top, float right, float bottom) {
        this.left = left;
        this.top = top;
        this.right = right;
        this.bottom = bottom;
     ...
     */
    public AxisValueRange(AxisValueRange v) {
        if (v == null) {
    ...
    public boolean equals(Object obj) {
        ...
        if (getClass() != obj.getClass())
            return false;
        AxisValueRange other = (AxisValueRange) obj;
     ...
     */
    public void set(AxisValueRange src) {
        this.left = src.left;
        this.top = src.top;
     ...
     */
    public boolean contains(AxisValueRange v) {
        // check for empty first
        return this.left < this.right && this.bottom < this.top
                // now check for containment
    ...
    /**
     * Update this AxisValueRange to enclose itself and the specified viewport. If the specified viewport is empty, nothing is
     ...
     */
    public void union(AxisValueRange v) {
     ...
     * intersection) else return false and do not change this viewport.
     */
    public boolean intersect(AxisValueRange v) {
        return intersect(v.left, v.top, v.right, v.bottom);
    }

    @Override
    public String toString() {
        return "AxisValueRange [left=" + left + ", top=" + top + ", right=" + right + ", bottom=" + bottom + "]";
    }
